...
public class CanvasController : MonoBehaviour
{
    ...
    private void TweenStarScale(int index)
    {
        ...
        Stars[index].transform.DOScale(new Vector3(1, 1, 1) + new Vector3(overshootAmount * 2, overshootAmount * 2, overshootAmount * 2), 0.7f)
            .SetEase(Ease.OutBounce)
            .OnComplete(() => Stars[index].transform.DOScale(new Vector3(1, 1, 1), 0.7f).SetEase(Ease.InOutQuad))
            .OnComplete(() =>
            {
                ScreenUIPanel.GetComponent<ScreenUI>().StarPlus();
                if (index < starsno - 1)
            {
